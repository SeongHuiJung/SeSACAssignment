...
class MapViewModel {
    ...
    var inputFoodType = CustomObservable(FoodType.all)
    
    // output
    var outputFoodList: CustomObservable<[Restaurant]> = CustomObservable([])
    
    // 각각의 탭 로드하지 않아도 바로 init 됨
    // 지도 뷰 뜨자마자 바로 모든 지점 뜨도록 bind 에 isfirstExecute을 true로 전달
    init() {
        inputFoodType.bind(isfirstExecute: true) { _ in
