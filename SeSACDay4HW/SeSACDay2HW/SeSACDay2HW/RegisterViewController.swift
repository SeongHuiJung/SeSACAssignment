...
class RegisterViewController: UIViewController {
    ...
    // 4. 추가정보 입력 스위치가 on 이라면, 위치 글자수는 50자 이하여야 합니다
    // 5. ID, PW, 닉네임은 반드시 작성되어야 하며 나머지 위치, 추천 코드는 선택사항입니다
    // 6. 상단의 5가지를 모두 만족해야 회원가입이 가능합니다
    ...
    func checkRegisterAble() -> Bool {
        ...
        if additionalInfoSwitch.isOn && locationTextField.text!.count > 50 { return false }
    ...
    @IBAction func switchTapped(_ sender: UISwitch) {
        if additionalInfoSwitch.isOn {
            ...
            locationCountLimitLabel.isHidden = false
        }
        else {
            ...
            locationCountLimitLabel.isHidden = true
        }
    }
